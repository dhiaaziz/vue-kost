            ...
            <div class="row justify-content-between">
              <div class="col-md-4">
                <div class="gap-2 d-grid d-md-block">
                  <router-link :to="{ name: 'Input Kamar' }" class="btn btn-sm btn-primary">
                    Tambah Kamar 
                  </router-link>
                </div>
              </div>
              ...
              <div class="mt-4 mt-md-0 col-12 col-md-4">
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" :placeholder="$store.state.isRTL ? 'أكتب هنا...' : 'Cari Kamar..'" v-model="search" @keyup="handleSearch"/>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="px-0 pt-0 pb-2 card-body">
            <div class="p-0 table-responsive">
              <!-- <div class="px-0 pt-0 pb-2 card-body"> -->
                ...
                <h6 v-else-if="kamar.length == 0" class="text-center">Tidak Memiliki kamar</h6>
                <div v-else class="p-0 table-responsive mx-1">
                  <table class="table mb-0 align-items-center text-center">
                    <thead>
                      <tr>
                        <th class=" text-uppercase text-secondary text-xs font-weight-bolder opacity-7">No</th>
                        <th class=" text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Build</th>
                    ...
                    <tbody>
                      <tr v-for="(item, key) in data" :key="key">
                        <td><p class="mb-0 text-sm font-weight-bold">{{ key+1+((page-1)*jumlah) }}</p></td>
                        <td><p class="mb-0 text-sm font-weight-bold">{{ item.build_name }}</p></td>
                        <td><p class="mb-0 text-sm font-weight-bold">{{ item.name }}</p></td>
                        <td><p class="mb-0 text-sm font-weight-bold">{{ item.size }}</p></td>
                        <td><p class="mb-0 text-sm font-weight-bold">{{ item.price }}</p></td>
                        <td><p class="mb-0 text-sm font-weight-bold">{{ item.username||'-' }}</p></td>
                        <td><p class="mb-0 text-sm font-weight-bold">{{ item.username?moment(item.start_kos).format('DD/MM/YYYY')+' - '+moment(item.end_kos).format('DD/MM/YYYY'):'-' }}</p></td>
                      </tr>
                    </tbody>
                  </table>
                  <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center">
                      <li v-if="page>2" class="page-item  disabled" @click="page=1">
                        <a class="page-link" aria-label="Previous">
                          <span aria-hidden="true">&laquo;</span>
                        </a>
                      </li>
                      <li v-if="page>1" class="page-item" @click="page--"><a class="page-link">{{page-1}}</a></li>
                      <li class="page-item active"><a class="page-link">{{page}}</a></li>
                      <li v-if="page<totalPage" class="page-item" @click="page++"><a class="page-link">{{page+1}}</a></li>
                      <li v-if="page<totalPage-1" class="page-item" @click="page=totalPage">
...
export default {
  ...
  async mounted() {
    ...
    try{
      ...
      if(kamar.length == 0) {vm.kamar = undefined; vm.tetangga = []}
      else{
  ...
  computed: {
    ...
    data(){
      ...
      return data.slice(this.jumlah * this.page - this.jumlah, this.jumlah * this.page)
    },
  },
  methods: {
